angular.module('cgNotify', []).factory('notify',['$timeout','$http','$compile','$templateCache','$rootScope',
    function($timeout,$http,$compile,$templateCache,$rootScope){
        ...
        var notify = function(args){
            ...
            orient = !angular.isUndefined(args.orient) ? args.orient : orient;
            ...
            $http.get(args.templateUrl,{cache: $templateCache}).success(function(template){
                ...
                templateElement.addClass('cg-notify-' + orient);
                ...
                var layoutMessages = function(){
                    ...
                    for(var i = messageElements.length - 1; i >= 0; i --){
                        ...
                        var left = currentX + width + shadowHeight;
                        if (element.attr('data-closing')){
                            top += 20;
                            left += 20;
                        } else {
                            currentY += height + verticalSpacing;
                            currentX += width + verticalSpacing;
                        }
                        if (orient == 'fromtop') {
                            element.css('top',top + 'px').css('margin-top','-' + (height+shadowHeight) + 'px').css('visibility','visible');
                        } else if (orient == 'frombottom') {
                            element.css('bottom',top + 'px').css('margin-bottom','-' + (height+shadowHeight) + 'px').css('visibility','visible');
                        } else if (orient == 'fromleft') {
                            element.css('left',left + 'px').css('margin-left','-' + (width+shadowHeight) + 'px').css('visibility','visible');
                        } else if (orient == 'fromright') {
                            element.css('right',left + 'px').css('margin-right','-' + (width+shadowHeight) + 'px').css('visibility','visible');
                        }
                        j ++;
                    }
                };
...
angular.module('cgNotify').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('angular-notify.html',
    ...
    "    $position === 'top-left' ? 'cg-notify-message-top-left' : '',\n" +
    "    $position === 'top-right' ? 'cg-notify-message-top-right' : '',\n" +
    "    $position === 'bottom-left' ? 'cg-notify-message-bottom-left' : '',\n" +
    "    $position === 'bottom-right' ? 'cg-notify-message-bottom-right' : '']\"\n" +
